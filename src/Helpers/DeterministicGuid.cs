...
{
    private readonly Random _rnd = new Random(1234); // Seeded (deterministic) random number generator.
    ...
    private string GetRandHexExp(long minIncl, int maxExclBase, int maxExclExponent)
    {
        return GetRandHex(minIncl, (long)Math.Pow(maxExclBase, maxExclExponent), maxExclExponent);
    }

    private string GetRandHex(long minIncl, long maxExcl, int digits)
    {
