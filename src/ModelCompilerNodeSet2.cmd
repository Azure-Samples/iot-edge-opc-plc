...
echo Model3 Filename : %filename3%
mkdir temp
...
IF modelName2 == "" (
    ...
    COPY %modelName1%.xml temp
    COPY %modelName2%.xml temp
    COPY %modelName3%.xml temp
    echo Building Nodeset2 %modelName1%.xml,%namespace1%,%prefix1% %modelName2%.xml,%namespace2%,%prefix2% %modelName3%.xml,%namespace3%,%prefix3%...
    %MODELCOMPILER% compile -version v104 -d2 "%MODELROOT%/temp/%filename1%.xml,%namespace1%,%prefix1%" -d2 "%MODELROOT%/temp/%filename2%.xml,%namespace2%,%prefix2%" -d2 "%MODELROOT%/temp/%filename3%.xml,%namespace3%,%prefix3%" -o2 "%MODELROOT%/"
)
